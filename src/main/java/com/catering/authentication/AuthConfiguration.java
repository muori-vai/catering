...
import static com.catering.model.Credentials.DEFAULT_ROLE;
...
public class AuthConfiguration extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
                ...
                .antMatchers(HttpMethod.GET, "/", "/index", "/login", "logout", "/register", "/css/**", "/images/**").permitAll()
                // chiunque (autenticato o no) può mandare richieste POST al punto di accesso per login e register 
                .antMatchers(HttpMethod.POST, "/login", "/register").permitAll()
                // solo gli utenti autenticati con ruolo ADMIN possono accedere a risorse con path /admin/**
                .antMatchers(HttpMethod.GET, "/admin/**", "/chefForm", "/buffetForm", "/piattoForm", "/ingredienteForm").hasAnyAuthority(ADMIN_ROLE)
                .antMatchers(HttpMethod.POST, "/admin/**", "/chef", "/ingrediente", "/buffet", "/piatto").hasAnyAuthority(ADMIN_ROLE)
                // solo gli utenti autenticati con ruolo DEFAULT o ADMIN possono accedere alle risorse di Catering
                .antMatchers(HttpMethod.GET, "/chef/**", "/buffet/**", "/piatto/**", "/ingrediente/**",
                			"/chefs", "/buffets", "/piatti", "/ingredienti").hasAnyAuthority(DEFAULT_ROLE, ADMIN_ROLE)
                .antMatchers(HttpMethod.POST, "/admin/**", "/chef", "/ingrediente", "/buffet", "/piatto").hasAnyAuthority(DEFAULT_ROLE, ADMIN_ROLE)
